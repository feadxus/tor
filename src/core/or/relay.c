                           ...
                           cell_direction_t cell_direction)
{
  ...
  if (recognized) {
    relay_msg_codec_t *codec = relay_msg_get_codec(circ, layer_hint);
    tor_assert(codec);
    ...
    /* Add the cell to our decoder and see if we have a valid message to
     * continue with else we are done, we need more cell(s). */
    if (!relay_msg_decode_cell(codec, cell)) {
      return -END_CIRC_REASON_TORPROTOCOL;
    }

    /* Go over all messages that are ready. Take them out of our decoder. */
    smartlist_t *messages = relay_msg_take_ready_msgs(codec);
    SMARTLIST_FOREACH_BEGIN(messages, const relay_msg_t *, msg) {

      if (circ->purpose == CIRCUIT_PURPOSE_PATH_BIAS_TESTING) {
        if (pathbias_check_probe_response(circ, msg) == -1) {
          pathbias_count_valid_cells(circ, msg);
        }
        relay_msg_free_messages(messages);
                               ...
                               const char *filename, int lineno))
{
  ...
  if (CIRCUIT_IS_ORIGIN(circ)) {
    ...
    cell_circid = circ->n_circ_id;
    cell_direction = CELL_DIRECTION_OUT;
  } else {
    tor_assert(! cpath_layer);
    cell_circid = TO_OR_CIRCUIT(circ)->p_circ_id;
    cell_direction = CELL_DIRECTION_IN;
  }
  /* This call guarantees to get a codec or else assert. */
  codec = relay_msg_get_codec(circ, cpath_layer);

  /* Set relay message with the data. */
  relay_msg_set(codec->relay_cell_proto, relay_command, stream_id,
                (const uint8_t *)payload, payload_len, &msg);
  /* And now, encode the message in cell(s). */
  if (BUG(!relay_msg_encode_msg(codec, &msg))) {
    circuit_mark_for_close(circ, END_CIRC_REASON_INTERNAL);
    goto end;
  }
  relay_msg_clear(&msg);

  log_debug(LD_OR, "Delivering %d relay message %s.", relay_command,
  ...
  /* Use thisflag to avoid sending multiple relay early cells within a set of
   * possibly fragmented cells. Send one, that is the first one and then the
   * rest will follow properly. */
  bool relay_early_sent = false;
  /* Take all cells that are ready and send them on the wire. */
  cells = relay_msg_take_ready_cells(codec);

  SMARTLIST_FOREACH_BEGIN(cells, cell_t *, cell) {
    /* Set the field in the cell for this batch. */
    cell->circ_id = cell_circid;
    ...
    if (cell_direction == CELL_DIRECTION_OUT) {
      ...
      if (!relay_early_sent &&
          origin_circ->remaining_relay_early_cells > 0 &&
      ...
      circuit_sent_valid_data(origin_circ, payload_len);
    }
  ...
  } SMARTLIST_FOREACH_END(cell);

  /* All cells were sent. Success. */
  ret = 0;

 end:
  if (cells) {
    SMARTLIST_FOREACH(cells, cell_t *, cell, tor_free(cell));
                                           ...
                                           crypt_path_t *cpath)
{
  ...
  /* At most how much would we like to send in this cell? This is guaranteed to
   * return at least RELAY_CELL_PADDING_GAP + 16 + 1 bytes. */
  size_t target_length =
    relay_msg_get_next_max_len(relay_msg_get_codec(circ, cpath),
                               RELAY_COMMAND_DATA);
